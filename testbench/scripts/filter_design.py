...
"""
class FIR():
    def __dB20(self, array):
        with np.errstate(divide='ignore'):
            return 20 * np.log10(array)

    def __fir_calc_filter(self, Fs, Fpb, Fsb, Apb, Asb, N):
        bands = np.array([0., Fpb/Fs, Fsb/Fs, .5])
        ...
        print("Rpb: %fdB" % (-self.__dB20(Rpb)))
        print("Rsb: %fdB" % -self.__dB20(Rsb))

        return (h, w, H, Rpb, Rsb, Hpb_min, Hpb_max, Hsb_max)
    
    def __init__(self, sf = 96000, pb = 26000, sb = 27000, pbdB = 0.1, sbdB = 120, order = 255):
        self.__sf = sf
        self.__pb = pb
        self.__sb = sb
        self.__pbdB = pbdB
...
class UpsamplerModel():
    def __init__(self, ssf = 44.1, uf = 64, stgs = 6):
        self.start_sf = ssf
        self.upsample_fact = uf
        self.stages = stgs
